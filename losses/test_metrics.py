...
class Tests_Metrics(unittest.TestCase):
    ...
    def test_custom_ssim(self):
        ...
        try:
            ...
            print(f'same_image: {same_images}')
            self.assertEqual(
                same_images, correct_output,
                'Custom SSIM gives wrong answer with the same image'
            )
            opposite_images = SSIM(fake_wrong_image, real_image)
            print(f'oppposite_image: {opposite_images}')
            self.assertEqual(
                round(opposite_images, 4), different_output,
                'Custom SSIM gives wrong answer with the opposite image'
            )
        except TypeError:
